// Точка входа в приложение SanyaPC.
// Инициализирует провайдеры состояния и запускает главный виджет приложения.
//(немного скостылили. Перенесли main_page.dart сюда т.к иначе не работал ui в localhost:8081)
import 'dart:ui';
...
void main() {
  runApp(MyApp());  // Запуск Flutter приложения
}

// Корневой виджет приложения
// Настраивает провайдеры состояния
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        // Регистрируем AuthProvider, который будет использовать UserService
        ChangeNotifierProvider(create: (context) => AuthProvider()),
        ChangeNotifierProvider(create: (_) => LikedProvider()),
        // Можно добавить другие провайдеры здесь
      ],
      child: MaterialApp(
        debugShowCheckedModeBanner: false, //убираем плашку дебага
        title: 'Main',
        theme: ThemeData(
          colorScheme: ColorScheme.fromSeed(seedColor: Colors.deepPurple),
          useMaterial3: true,
        ),
        home: const MainPage(), //корневая страница
      ),
    );
  }
}
